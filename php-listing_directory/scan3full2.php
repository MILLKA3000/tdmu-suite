                        ...
                        if ($path->isFile()) {
                            ...
                            if ((stripos($curr_dir, $skip_dir1)===false)&&(stripos($curr_dir, $skip_dir2)===false)){
                                ...
                                if ((stripos($file,".htm"))||(stripos($file,".html"))||(stripos($file,".ppt"))||(stripos($file,".pptx"))||(stripos($file,".pdf"))){
                                    ...
                                    $tmp_subject = '';
                                    $tmp_dirname_arr = $scanutils->getSubDirFoolproof($curr_dir, $base_path);
                                    $max_add_dir_depth = 3;
                                    $curr_add_depth = 0;
                                    foreach($tmp_dirname_arr as $subdir_id=>$subdir_name){
                                        ...
                                        if ($curr_add_depth<=$max_add_dir_depth){
                                            if ($subdir_name == "ptn") {
                                                $curr_term = "ПТН. Предмет (чи/і) курс: ";
                                            } elseif ($subdir_name == "ntn") {
                                                $curr_term = "НТН. Предмет (чи/і) курс: ";
                                            } else {
                                    ...
                                    if (!isset($res_count5[$fkt_id][$spec_id][$curr_subject][$lang_id][$cat_id][$curr_dir])){
                                        $res_count5[$fkt_id][$spec_id][$curr_subject][$lang_id][$cat_id][$curr_dir]= 1;
                                    } else {
                                        $res_count5[$fkt_id][$spec_id][$curr_subject][$lang_id][$cat_id][$curr_dir]++;
                                    }    
...
foreach ($doc_4_spec_by_fkt as $fkt_id=>$fkt_spec){
    ...
    foreach ($fkt_spec as $spec_id=>$spec_name){
        ...
        if (isset($res_count5[$fkt_id][$spec_id])){
            foreach ($res_count5[$fkt_id][$spec_id] as $curr_subj=>$subj_fdata){
                echo '</tr><tr><td colspan="7"><center>'.$curr_subj."</center></td>";
                //process all languages
                foreach ($doc_2_lang as $lang_id=>$lang_name){
                    ...
                    foreach ($doc_1_categories as $cat_id=>$cat_name){
                        ...
                        if (isset($res_count5[$fkt_id][$spec_id][$curr_subj][$lang_id][$cat_id])){
                
                            foreach ($res_count5[$fkt_id][$spec_id][$curr_subj][$lang_id][$cat_id] as $flpath=>$flcount){
