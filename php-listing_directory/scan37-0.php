                        ...
                        if ($path->isFile()) {
                            ...
                            if ((stripos($curr_dir, $skip_dir1)===false)&&(stripos($curr_dir, $skip_dir2)===false)){
                                ...
                                if ((stripos($file,".htm"))||(stripos($file,".html"))||(stripos($file,".ppt"))||(stripos($file,".pptx"))||(stripos($file,".pdf"))){
                                    ...
                                    if (!isset($res_count5[$fkt_id][$spec_id][$lang_id][$curr_subject][$cat_id][$curr_dir])){
                                        $res_count5[$fkt_id][$spec_id][$lang_id][$curr_subject][$cat_id][$curr_dir]= 1;
                                    } else {
                                        $res_count5[$fkt_id][$spec_id][$lang_id][$curr_subject][$cat_id][$curr_dir]++;
                                    }    
...
foreach ($doc_4_spec_by_fkt as $fkt_id=>$fkt_spec)
{
    ...
    foreach ($fkt_spec as $spec_id=>$spec_name)
    {
        ...
        foreach ($doc_2_lang as $lang_id=>$lang_name)
        {
            ...
            //$row_flcount = 0;
            if (isset($res_count5[$fkt_id][$spec_id][$lang_id])){
                foreach ($res_count5[$fkt_id][$spec_id][$lang_id] as $curr_subj=>$subj_fdata){
                    echo "</tr><tr><td>".$doc_2_lang[$lang_id]."</td>";
                    echo '<td colspan="5">'.$curr_subj."</td><td>-</td>";
                    echo "</tr><tr><td>".$doc_2_lang[$lang_id]."</td>";
                    $row_flcount = 0;
                    ...
                    foreach ($doc_1_categories as $cat_id=>$cat_name){
                       ...
                       // foreach ($res_count5[$fkt_id][$spec_id][$lang_id][$curr_subj] as $flpath=>$flcount){
                
                            foreach ($res_count5[$fkt_id][$spec_id][$lang_id][$curr_subj][$cat_id] as $flpath=>$flcount){
